...
for i in range(0, 192, 24):
    ...
    url = f"https://www.skiddle.com/festivals/search/?ajaxing=1&sort" \
          f"=0&fest_name=&from_date=24%20Jan%202021&to_date=&where%5B" \
          f"%5D=2&where%5B%5D=3&where%5B%5D=4&where%5B%5D=6&where%5B%" \
          f"5D=7&where%5B%5D=8&where%5B%5D=9&where%5B%5D=10&maxprice=500&" \
          f"o={i}&bannertitle=May"
...
for url in fests_urls_list:
    ...
    try:
        ...
        fest_location_url = "https://www.skiddle.com" + \
                            fest_info_block.find("a", class_="tc-white")\
                            .get("href")
        ...
        contact_details = soup.find("h2", string="Venue contact "
                                                 "details and info").find_next()
